...
boolean stopsData;
PImage busStops, mainPageImg, background4Home, pie, busRoute, back, cloud, mapImage,Generalinfo;
Data myDataPoints;
...
/*
 ...
 Generalinfo= loadImage("Generalinfo.png");   //pie chart button image - http://www.allthingsclipart.com/apple.pie.clipart.htm
  ...
  nextWidget=new WidgetButton(220, 360, 100, 90, "routeData", color(255), color(255), stdFont, BUS_QUERY);
...
void draw()                             
{
  ...
  case mainQuery:                                                  //Screen 1 stuff
    ...
    image(Generalinfo, width/2-80, 90, 110, 100);         // widgets are in                           please do not
   //if it needs to be changed                  if statements around 
    image(cloud, 740, 305, 310, 200);        // change the (x,y,width,hight)               leave in this order ,unless
    image(busRoute, 850, 360, 100, 90);   // of the widgets in setup                    map starts to glitch 
    image(cloud, 120, 305, 310, 200);         // Ezi
    image(busStops, 220, 360, 100, 90);       //}
    break;
  case query1:                                                      
